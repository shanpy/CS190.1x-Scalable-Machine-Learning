...
aSparse = SparseVector(aDense.size,{i:aDense[i] for i in np.nonzero(aDense)[0]})

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(bDense.size,{i:bDense[i] for i in np.nonzero(bDense)[0]})
...
# TODO: Replace <FILL IN> with appropriate code
sampleOneOHEFeatManual = SparseVector(7, {i:np.array([0,0,1,1,0,0,0])[i] for i in np.nonzero(np.array([0,0,1,1,0,0,0]))[0]})
sampleTwoOHEFeatManual = SparseVector(7, {i:np.array([0,1,0,0,1,1,0])[i] for i in np.nonzero(np.array([0,1,0,0,1,1,0]))[0]})
sampleThreeOHEFeatManual = SparseVector(7, {i:np.array([1,0,0,1,0,0,1])[i] for i in np.nonzero(np.array([1,0,0,1,0,0,1]))[0]})
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    newRawFeats = map(lambda x: OHEDict.get(x), rawFeats)
    newArray = [1] * len(OHEDict)
    return SparseVector(numOHEFeats, zip(newRawFeats, newArray))
...
def parsePoint(point):
    ...
    """
    final = point.split(',')[1:]
    return list(zip(xrange(len(final)), final))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    newRawFeats = map(lambda x: OHEDict.get(x), rawFeats)
    result = [x for x in newRawFeats if x is not None]
    return SparseVector(numOHEFeats, zip(result, [1] * len(result)))
...
def computeSparsity(data, d, n):
    ...
    """
    total = data.map(lambda x: len(x.features.values)/float(d)).sum()
