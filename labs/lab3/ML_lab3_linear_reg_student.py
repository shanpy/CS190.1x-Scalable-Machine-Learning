...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    return LabeledPoint(line.split(',')[0], line.split(',')[1:len(line)-1])

parsedSamplePoints = sc.parallelize(samplePoints).map(lambda x: parsePoint(x)).collect()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    test1 = labelsAndPreds.map(lambda x: (x))
    test2 = list(squaredError(a,b) for (a,b) in test1.collect())
    return np.sqrt(np.mean(np.array(test2)))
...
def getLabeledPrediction(weights, observation):
    ...
    return (observation.label,prediction)
...
def twoWayInteractions(lp):
    ...
    """
    newfeatures = map(lambda x: x[0]*x[1], list(itertools.product(lp.features, lp.features)))
    newstack = np.hstack((lp.features, newfeatures))
    return LabeledPoint(lp.label, newstack)
