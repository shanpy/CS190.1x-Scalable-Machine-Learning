...
def estimateCovariance(data):
    ...
    """
    meanvalue = data.mean()
    meanRDD = data.map(lambda x: x-meanvalue)
    return meanRDD.map(lambda x: np.outer(x,x)).sum()/(correlatedDataZeroMean.count())
...
inds = np.argsort(eigVecs[:,0])
topComponent = eigVecs[inds.max()]
